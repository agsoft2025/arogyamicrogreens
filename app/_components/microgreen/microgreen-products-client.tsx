...
type MicrogreenProductsClientProps = {
  pageProducts: MicrogreenProduct[];
  seedImages: Record<string, StaticImageData>;
  currentPage: number;
  totalPages: number;
};

export function MicrogreenProductsClient({
  pageProducts,
  seedImages,
  currentPage,
  totalPages,
}: MicrogreenProductsClientProps) {
      ...
      <div className="mx-auto w-full max-w-5xl px-6">
        ...
        <div className="grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
          {pageProducts.map((product, index) => (
            <FadeIn key={product.id} delay={0.035 * (index % 6)} distance={18}>
              <article className="group">
                <Link href={`/microgreen/${product.slug}`} className="block">
