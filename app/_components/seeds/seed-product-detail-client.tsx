...
type SeedProductDetailClientProps = {
  product: SeedProduct;
};

export function SeedProductDetailClient({
  product,
}: SeedProductDetailClientProps) {
  ...
  const growingParameters = [
    `Seed Soaking: ${product.packSizeLabel === "100 Gms" ? "Not Required" : "As needed"}`,
    `Germination Time: ${product.growingTime}`,
    "Blackout Period: 2-3 days",
    `Harvest Time: ${product.growingTime}`,
    "Watering: Light misting daily",
  ];
  ...
  const growingSteps = [
    {
      ...
      description:
        "Fill a shallow tray with cocopeat or another growing medium and level the surface evenly.",
    },
    {
      title: "Sow the seeds:",
      description: `Evenly sprinkle ${product.baseName} seeds across the tray without overcrowding.`,
    },
    {
      title: "Mist gently:",
      description:
        "Lightly mist the tray to keep the growing medium evenly moist.",
    },
    {
      title: "Blackout phase:",
      description:
        "Cover the tray and keep it in a dark place for 2-3 days to allow seeds to germinate.",
    },
    {
      title: "Move to light:",
      description:
        "Once sprouts appear, remove the cover and place the tray under indirect sunlight or grow lights.",
    },
    {
      title: "Maintain moisture:",
      description:
        "Lightly mist the tray once or twice daily to keep the medium moist.",
    },
    {
      title: "Ensure airflow:",
      description:
        "Keep the tray in a well-ventilated area to support healthy growth.",
    },
    {
      title: "Harvest:",
      description:
        "Harvest once the microgreens have developed a healthy canopy and reached usable height.",
    },
  ];
  ...
  const renderInfoSection = (
    ...
    return (
      <article key={section.title} className="border border-white/15">
        <button
          type="button"
          onClick={() =>
        ...
        {isOpen ? (
          <div className="border-t border-white/15 px-5 py-5">
            {section.description ? (
              <p className="text-[15px] font-medium leading-8 text-white/95">
                {section.description}
              </p>
            ) : null}
            ...
            {section.items && section.items.length > 0 ? (
              <ul className="mt-4 space-y-2 text-[15px] font-medium leading-7 text-white/90">
            ...
            {section.conclusion ? (
              <p className="mt-5 text-[15px] font-medium leading-8 text-white/95">
                {section.conclusion}
              </p>
            ) : null}
          </div>
      ...
      <section className="bg-[#f5f5f3] px-5 py-10 md:py-14">
        <div className="mx-auto max-w-6xl rounded-[18px] bg-white px-6 py-5 shadow-[0_28px_80px_rgba(24,28,24,0.12)] md:px-8 md:py-6">
          ...
          <div className="mt-6 grid gap-12 md:grid-cols-[420px_minmax(0,1fr)] md:items-start">
            ...
            <div className="max-w-[500px] pt-1">
              <h1 className="font-serif text-xl font-bold leading-1 text-[#2c2b31] md:text-[58px] md:tracking-[-0.02em]">
                {product.name}
              </h1>

              <div className="mt-3 space-y-4 text-[18px] font-medium text-[#667b9c]">
                {product.description.map((paragraph) => (
                  <p key={paragraph}>{paragraph}</p>
                ))}
              </div>

              <p className="mt-8 text-xl font-extrabold leading-none text-[#77b03d]">
                {formatSeedPrice(product.price)}
              </p>
              <p className="mt-2 text-[22px] italic tracking-[0.02em] text-[#7b6c63]">
        ...
        <div className="mx-auto mt-12 max-w-5xl rounded-[22px] bg-[#3b3838] px-6 py-7 text-white shadow-[0_28px_48px_rgba(0,0,0,0.28)] md:px-12 md:py-12">
          <div className="grid gap-8 md:grid-cols-[1.05fr_0.95fr] md:gap-10">
            <div className="space-y-6 text-[16px] font-medium leading-10 text-white">
              {product.description.map((paragraph) => (
                <p key={paragraph}>{paragraph}</p>
              ))}
              <p>
                Microgreens de Manikonda provides{" "}
                <strong className="font-extrabold">
                  high-quality {product.baseName} seeds with excellent
                  germination rates
                </strong>
                , ensuring reliable growth for both beginners and experienced
                growers.
              </p>
            </div>

            <div className="space-y-4">
              <article className="border border-white/15 px-5 py-5">
                <h2 className="text-[14px] font-extrabold uppercase tracking-[0.08em] text-[#a9d8b1]">
                  Flavor Profile
                </h2>
                <p className="mt-5 text-[15px] font-medium leading-8 text-white/95">
                  {product.baseName} microgreens offer a{" "}
                  <strong className="font-extrabold">
                    {product.flavorProfile.toLowerCase()}
                  </strong>
                  .
                </p>
                <p className="mt-4 text-[15px] font-medium text-white/95">
                ...
                <ul className="mt-4 space-y-2 text-[15px] font-medium leading-7 text-white/90">
                  <li>• {product.bestFor[0] ?? "Tender leaves"}</li>
                  <li>• {product.bestFor[1] ?? "Balanced earthy notes"}</li>
                  <li>• {product.bestFor[2] ?? "Clean finish"}</li>
                </ul>
                <p className="mt-5 text-[15px] font-medium leading-8 text-white/95">
                  They work well with{" "}
                  <strong className="font-extrabold">
                    {product.bestFor.slice(0, 4).join(", ").toLowerCase()}
                  </strong>
                  .
                </p>
              </article>

              <div className="flex items-center justify-between border border-white/15 px-5 py-4">
                <h3 className="text-[14px] font-extrabold uppercase tracking-[0.08em] text-[#a9d8b1]">
