    ...
    <main className="min-h-screen bg-[#fafaf4]">
      <div className="max-w-[1280px] mx-auto px-5 md:px-16 py-10">
        ...
        >
          ...
          <ChevronRight />
          <Link href="/microgreens" className="hover:text-[#032616] transition-colors">Microgreens</Link>
          <ChevronRight />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          ...
          >
            <div className="relative aspect-square bg-[#eeeee9] rounded-2xl overflow-hidden">
              ...
              >
                <svg
                  className={`w-5 h-5 transition-all duration-200 ${inWishlist ? "text-[#ba1a1a] scale-110" : "text-[#424843]"}`}
                ...
                {images.map((img, i) => (
                  <motion.button
                    ...
                    className={`relative w-20 h-20 shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                      activeImg === i ? "border-[#032616]" : "border-[#e3e3dd] hover:border-[#386b00]"
                    }`}
                    aria-label={`View image ${i + 1}`}
                  >
                    <img src={getProductImageUrl(img)} alt={`${product.name} ${i + 1}`} className="w-full h-full object-cover" />
                  </motion.button>
                ))}
              </div>
            )}
            ...
            {product.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {product.tags.slice(0, 4).map((tag) => (
                  <span key={tag} className="bg-[#f4f4ee] text-[#424843] text-[10px] font-bold tracking-widest uppercase font-[var(--font-work-sans)] px-3 py-1 rounded-full border border-[#e3e3dd]">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2 mb-8">
              {product.stock > 10 ? (
                <>
                  ...
                  <span className="text-[11px] font-bold tracking-widest uppercase font-[var(--font-work-sans)] text-[#386b00]">In Stock</span>
                </>
              ) : product.stock > 0 ? (
                <>
                  <span className="w-2 h-2 rounded-full bg-[#f4a800]" />
                  <span className="text-[11px] font-bold tracking-widest uppercase font-[var(--font-work-sans)] text-[#f4a800]">Only {product.stock} left</span>
                </>
              ) : (
                <>
                  <span className="w-2 h-2 rounded-full bg-[#ba1a1a]" />
                  <span className="text-[11px] font-bold tracking-widest uppercase font-[var(--font-work-sans)] text-[#ba1a1a]">Out of Stock</span>
                </>
              )}
            </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href="/microgreens">
            <motion.span
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.97 }}
              className="block w-full bg-[#032616] hover:bg-[#0a3d20] text-white font-bold text-[11px] tracking-[0.14em] uppercase font-[var(--font-work-sans)] py-3 rounded-xl text-center">
              Browse Microgreens
            </motion.span>
          </Link>
        </div>
